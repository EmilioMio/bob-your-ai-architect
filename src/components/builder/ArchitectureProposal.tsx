...
function generateFileContents(architecture: GeneratedArchitecture, formData: ProjectFormData): Record<string, string> {
  ...
  const stubForFile = (path: string, description?: string) => {
    const ext = getExt(path);
    const desc = (description || "").trim();

    if (ext === ".md") return `# ${path.split("/").pop()}\n\n${desc ? desc + "\n" : ""}`;
    if (ext === ".json") return `{\n}\n`;
    if (ext === ".yml" || ext === ".yaml") return `# ${desc || "Configuration"}\n`;
    if (ext === ".sql") return `-- ${desc || "Database schema / migrations"}\n`;
    if (ext === ".ts" || ext === ".tsx" || ext === ".js" || ext === ".jsx")
      return `/**\n * ${desc || "TODO"}\n */\n\nexport {};\n`;
    if (ext === ".env" || path.endsWith(".env.example")) return `# ${desc || "Environment variables"}\n`;

    // default: empty file (still useful to force the file into the zip)
    return desc ? `${desc}\n` : "";
  };

  const addTreeToFiles = (node: ArchitectureFile, parentPath = "") => {
    const base = parentPath ? `${parentPath}/` : "";
    const currentPath = normalizePath(`${base}${node.name}`);

    if (node.type === "folder") {
      const children = node.children || [];

      // If folder is empty, add a placeholder so the folder exists in the ZIP.
      // (JSZip won't preserve empty folders unless there's at least one entry.)
      if (children.length === 0) {
        files[normalizePath(`${currentPath}/.gitkeep`)] = "";
        return;
      }

      for (const child of children) {
        addTreeToFiles(child, currentPath);
      }
      return;
    }

    // file node
    files[currentPath] = stubForFile(currentPath, node.description);
  };

  // --- 1) Always include these top-level docs ---
  files["README.md"] = `# ${architecture.projectName}
...
## Architecture
See [ARCHITECTURE.md](./ARCHITECTURE.md) for guidelines.
`;

  files["ARCHITECTURE.md"] = `# Architecture Guidelines
...
`;
  ...
  // --- 3) Add the AI-proposed file structure into the ZIP ---
  // We create three top-level folders so it's clear what belongs where.
  if (architecture.fileStructure.frontend) addTreeToFiles(architecture.fileStructure.frontend, "");
  if (architecture.fileStructure.backend) addTreeToFiles(architecture.fileStructure.backend, "");
  if (architecture.fileStructure.mobile) addTreeToFiles(architecture.fileStructure.mobile, "");

  // --- 4) Provide a couple useful boilerplate root files ---
  // (Only if the architecture tree didn’t already specify them)
  if (!files[".gitignore"]) {
...
`;
  ...
  // Keep package.json basic; if your architecture tree includes a package.json,
  // this will be overwritten only if that entry is added later (it isn't).
  // So: only add it if no package.json exists in the generated structure.
