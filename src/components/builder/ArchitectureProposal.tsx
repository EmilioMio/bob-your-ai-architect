...
import { 
  ...
  Database, Server, Code2, RefreshCw, Save, FolderOpen, FileSearch, Package
} from 'lucide-react';
...
/**
 * Recursively adds file structure to ZIP
 * Creates folders and files with minimal content
 */
function addStructureToZip(
  zip: JSZip,
  node: ArchitectureFile,
  basePath: string = ''
): void {
  if (!node) return;
  ...
  if (node.type === 'folder') {
    if (!node.children || node.children.length === 0) {
      zip.file(`${currentPath}/.gitkeep`, '# Placeholder for empty directory\n');
    } else {
      node.children.forEach((child) => {
        addStructureToZip(zip, child, currentPath);
      });
    }
  } else if (node.type === 'file') {
    const content = generateMinimalFileContent(node.name, node.description);
    zip.file(currentPath, content);
  }
}

/**
 * Generate minimal file content based on file extension
 */
function generateMinimalFileContent(fileName: string, description?: string): string {
  ...
  switch (ext) {
    case 'py':
      return `# ${fileName}
# Generated by Bob - AI Code Architect
${desc}
...
/**
 * Generate SQL schema from database config
 */
function generateDatabaseSQL(database: GeneratedArchitecture['database']): string {
  if (!database || !database.tables) return '';
  
  let sql = `-- Database Schema: ${database.type}
-- Generated by Bob - AI Code Architect
...
`;
    ...
    const columns = table.columns.map((col) => {
      ...
      if (col.foreignKey) constraints.push(`REFERENCES ${col.foreignKey}`);
  ...
  if (database.relationships && database.relationships.length > 0) {
    ...
    database.relationships.forEach((rel) => {
      sql += `-- ${rel.from}.${rel.foreignKey} -> ${rel.to} (${rel.type})\n`;
    });
  }
...
/**
 * Generate config files for the project
 */
function generateConfigFiles(architecture: GeneratedArchitecture): Record<string, string> {
...
export function ArchitectureProposal({ formData, agents, conversationHistory, onBack, onLoadProject }: ArchitectureProposalProps) {
  ...
  const handleGenerateFiles = async () => {
    ...
    try {
      ...
      // Add blueprint.json (architecture metadata)
      ...
      // CRITICAL: Add ACTUAL file structures from architecture
      
      // Add frontend structure if it exists
      if (architecture.fileStructure?.frontend) {
        console.log('Generating frontend structure...');
        addStructureToZip(zip, architecture.fileStructure.frontend, '');
        setGenerationProgress(40);
        await new Promise(resolve => setTimeout(resolve, 200));
      }

      // Add backend structure if it exists
      if (architecture.fileStructure?.backend) {
        console.log('Generating backend structure...');
        addStructureToZip(zip, architecture.fileStructure.backend, '');
        setGenerationProgress(60);
        await new Promise(resolve => setTimeout(resolve, 200));
      }

      // Add mobile structure if it exists
      if (architecture.fileStructure?.mobile) {
        console.log('Generating mobile structure...');
        addStructureToZip(zip, architecture.fileStructure.mobile, '');
        setGenerationProgress(70);
        await new Promise(resolve => setTimeout(resolve, 200));
      }

      // Add database schema SQL file if database exists
      if (architecture.database?.required && architecture.database.tables) {
        const sqlContent = generateDatabaseSQL(architecture.database);
      ...
      const configFiles = generateConfigFiles(architecture);
      ...
      const safeName = architecture.projectName.toLowerCase().replace(/\s+/g, '-');
      saveAs(blob, `${safeName}-complete.zip`);
  ...
  const handleDownloadFrontend = async () => {
    if (!architecture?.fileStructure?.frontend) return;
    ...
    try {
      ...
      zip.file('README.md', `# ${architecture.projectName} - Frontend\n\n${architecture.summary}\n\nFrontend structure generated by Bob AI.`);
      setGenerationProgress(80);
      
      const blob = await zip.generateAsync({ type: 'blob' });
      const safeName = architecture.projectName.toLowerCase().replace(/\s+/g, '-');
      saveAs(blob, `${safeName}-frontend.zip`);
  ...
  const handleDownloadBackend = async () => {
    if (!architecture?.fileStructure?.backend) return;
    ...
    try {
      ...
      zip.file('README.md', `# ${architecture.projectName} - Backend\n\n${architecture.summary}\n\nBackend structure generated by Bob AI.`);
      ...
      const blob = await zip.generateAsync({ type: 'blob' });
      const safeName = architecture.projectName.toLowerCase().replace(/\s+/g, '-');
      saveAs(blob, `${safeName}-backend.zip`);
  ...
  const handleDownloadDatabase = async () => {
    if (!architecture?.database?.required) return;
    ...
    try {
      ...
      const blob = await zip.generateAsync({ type: 'blob' });
      const safeName = architecture.projectName.toLowerCase().replace(/\s+/g, '-');
      saveAs(blob, `${safeName}-database.zip`);
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="center" className="w-56">
            <DropdownMenuItem 
              ...
              disabled={!architecture?.fileStructure?.frontend || isGeneratingFiles}
            ...
            <DropdownMenuItem 
              ...
              disabled={!architecture?.fileStructure?.backend || isGeneratingFiles}
              className="cursor-pointer"
            >
            ...
            <DropdownMenuItem 
              ...
              disabled={!architecture?.database?.required || isGeneratingFiles}
              className="cursor-pointer"
            >
